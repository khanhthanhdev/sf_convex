...
class GeminiWrapper:
    ...
    def __init__(
        ...
        Args:
            ...
            api_key: Gemini API key (if not provided, will use configuration or env var)
        """
        ...
        if _config_manager:
            try:
                provider_config = _config_manager.get_provider_config('gemini')
            except Exception as e:
                print(f"Warning: Could not load Gemini configuration: {e}")

        # Get API key with configuration fallback
        resolved_api_key = (
            api_key or 
        ...
        if not resolved_api_key:
            raise ValueError("No API_KEY found. Please set the `GEMINI_API_KEY` or `GOOGLE_API_KEY` environment variable, configure it in the system, or pass api_key parameter.")
